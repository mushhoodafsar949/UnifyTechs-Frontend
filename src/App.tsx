    ...
    <section className="landing-banner" role="dialog" aria-modal="true" aria-labelledby="landing-banner-title">
      <div className="landing-banner__panel">
        ...
        <div className="landing-banner__content" key={`content-${banner.image}`}>
          <div className="landing-banner__brand"><img src="/brand/unify-techs-approved-mark.png" alt="" /><span>Unify Techs</span></div>
    ...
    <header className="nav-shell">
      <div className="site-width nav-inner">
        <a className="brand" href="/" aria-label="Unify Techs home">
          <img src="/brand/unify-techs-approved-mark.png" alt="" width="42" height="42" />
    ...
    <footer className="footer">
      <div className="site-width footer-grid">
        <div className="footer-brand">
          <a className="brand brand--footer" href="/"><img src="/brand/unify-techs-approved-mark.png" alt="" width="46" height="46" /><span>Unify Techs</span></a>
          <p>Process automation, digital products, and cloud foundations designed as one connected system.</p>
          <a href="https://www.linkedin.com/company/unifytechs" target="_blank" rel="noreferrer">LinkedIn</a>
        </div>
...
function HomePage() {
  return <>
    <main>
      ...
      <section className="healthcare-feature section-pad"><div className="site-width healthcare-grid"><div><span className="eyebrow eyebrow--light">Industry application</span><h2>Healthcare claims operations, repositioned where they belong.</h2><p>The healthcare expertise already developed by Unify Techs remains available as a focused application of Process Automation—not as the company’s entire identity.</p><a className="text-link light" href="/process-automation/healthcare">Explore healthcare automation <ArrowRight /></a></div><div className="healthcare-list">{healthcareSolutions.slice(0, 4).map(([title], i) => <span key={title}><b>0{i + 1}</b>{title}</span>)}</div></div></section>
      <section className="patterns section-pad"><div className="site-width"><div className="section-heading split"><div><span className="eyebrow">Selected solution patterns</span><h2>Useful systems, without invented case studies.</h2></div><p>Until verified customer information is available, project examples are presented as solution patterns and example architectures.</p></div><div className="pattern-grid">{[
        ['Document intake & processing', 'Capture, validation, exception review, and structured delivery.'], ['Client onboarding automation', 'Requests, evidence, approvals, provisioning, and status visibility.'], ['Operational portals', 'One place for customers and teams to manage work and decisions.'], ['System & API integrations', 'Reliable data movement with retries, audit logs, and ownership.'], ['SaaS platforms', 'Focused products built around a valuable repeatable workflow.'], ['Cloud modernization', 'A staged path to more observable and maintainable infrastructure.']
      ].map(([title, body], i) => <article key={title}><span>{String(i + 1).padStart(2, '0')}</span><h3>{title}</h3><p>{body}</p><a href="/work">View pattern <ArrowRight size={16} /></a></article>)}</div></div></section>
      <section className="engage section-pad"><div className="site-width"><div className="section-heading"><span className="eyebrow">How we engage</span><h2>Start with the scope that matches the problem.</h2></div><div className="engage-grid">{[['Automation Assessment','Map a manual process, identify bottlenecks, and produce an implementation roadmap.'],['Project Delivery','End-to-end implementation for a defined business or technical objective.'],['Technical Delivery Partnership','Engineering and infrastructure support for agencies, consultancies, and product teams.']].map(([title,body],i)=><article key={title}><span>0{i+1}</span><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>
      <section className="insights-preview section-pad"><div className="site-width"><div className="section-heading split"><div><span className="eyebrow">Insights</span><h2>Practical thinking for operational systems.</h2></div><a className="text-link" href="/insights">View all insights <ArrowRight /></a></div><div className="insight-grid">{['How to Identify a Process Worth Automating','Why Automating a Broken Workflow Makes It Worse','From Spreadsheet to Operational System'].map((title,i)=><a href="/insights" key={title}><span>{['PROCESS AUTOMATION','WORKFLOW DESIGN','SYSTEM DESIGN'][i]}</span><h3>{title}</h3><p>Read article <ArrowRight size={16}/></p></a>)}</div></div></section>
...
function WorkPage() {
  ...
  return <main>
    ...
    <section className="work-patterns section-pad"><div className="site-width"><div className="section-heading split"><div><span className="eyebrow">Additional solution patterns</span><h2>Common systems we can help define and deliver.</h2></div><p>Each engagement begins with discovery. Architecture, controls, integrations, and scope are adapted to the organization.</p></div><div className="pattern-grid pattern-grid--work">{[['Approval workflows','Request → policy check → owner review → approval → audit record'],['Client onboarding','Intake → evidence → verification → provisioning → status visibility'],['Operational portals','Customer requests → work queues → decisions → notifications'],['API integrations','Canonical data → adapters → retries → reconciliation → logging'],['Cloud modernization','Assessment → target architecture → staged migration → observability'],['Mobile products','Product strategy → UX → engineering → release → iteration']].map(([t,b],i)=><article key={t}><span>PATTERN {String(i+1).padStart(2,'0')}</span><h3>{t}</h3><p>{b}</p><a href="/contact">Discuss this pattern <ArrowRight size={16}/></a></article>)}</div></div></section>
    <section className="work-method section-pad"><div className="site-width"><div className="section-heading"><span className="eyebrow eyebrow--light">How we present the work</span><h2>Evidence over theatre.</h2><p>Professional delivery is visible in the decisions, artifacts, controls, and operating clarity—not in inflated claims.</p></div><div className="work-method__grid">{[['Discovery evidence','Process maps, constraints, owners, failure points, and defined outcomes.'],['System definition','Experience flows, architecture boundaries, integration contracts, and operational responsibilities.'],['Delivery evidence','Working increments, quality checks, release controls, documentation, and measurable acceptance criteria.'],['Responsible publishing','Only approved customer names, verified outcomes, and properly contextualized metrics are presented as case studies.']].map(([title,body],i)=><article key={title}><span>{String(i+1).padStart(2,'0')}</span><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>
    <PageCta />
  </main>
}
...
function InsightsPage() {
  const posts = [
    {cat:'Process Automation',title:'How to identify a process worth automating',summary:'Start with repetitive work that has stable rules, measurable volume, clear ownership, and an expensive failure mode.',takeaway:'A five-signal opportunity test'},
    {cat:'Workflow Design',title:'Why automating a broken workflow makes it worse',summary:'Automation amplifies the process it receives. Remove redundant approvals, unclear decisions, and unnecessary handoffs first.',takeaway:'Simplify before you automate'},
    {cat:'System Design',title:'From spreadsheet to operational system',summary:'A spreadsheet becomes risky when it is simultaneously a database, task queue, approval tool, and reporting layer.',takeaway:'Know when the operating model has outgrown the file'},
    {cat:'Web & App Engineering',title:'When your business needs a client portal',summary:'A portal is valuable when customers repeatedly request status, submit structured information, or collaborate across a long-running service.',takeaway:'Design around customer jobs, not a feature list'},
    {cat:'Cloud Architecture',title:'What good observability looks like for a growing platform',summary:'Useful telemetry connects technical signals to customer impact and gives an owner enough context to act quickly.',takeaway:'Logs, metrics, traces, and accountable response'},
    {cat:'Product Strategy',title:'Off-the-shelf software or a custom application?',summary:'Buy for standard capabilities. Build when the workflow creates real differentiation, carries unusual constraints, or must connect fragmented systems.',takeaway:'A practical build-versus-buy framework'}
  ]
  return <main>
    ...
    <section className="insight-library section-pad"><div className="site-width"><div className="section-heading split"><div><span className="eyebrow">The field guide</span><h2>Ideas your team can use in the next conversation.</h2></div><p>Concise perspectives grounded in process clarity, maintainable architecture, and responsible delivery.</p></div><div className="insight-list">{posts.map((post,i)=><article key={post.title}><div className="insight-card__top"><span>{post.cat}</span><b>{String(i+1).padStart(2,'0')}</b></div><h3>{post.title}</h3><p>{post.summary}</p><div className="insight-takeaway"><Check size={17}/><span>{post.takeaway}</span></div></article>)}</div></div></section>
...
function PageCta(){return <section className="page-cta"><div className="site-width page-cta__panel"><div className="page-cta__copy"><span className="eyebrow eyebrow--light">A practical next step</span><h2>Bring us the workflow, not a preselected tool.</h2><p>We’ll help define what should be simplified, connected, automated, or built.</p></div><a className="button button--white" href="/contact">Discuss your project <ArrowRight/></a></div></section>}
...
function App() {
  ...
  useEffect(() => {
    ...
    const fallback: [string, string] = path.startsWith('/process-automation/healthcare/')
