...
function HomePage() {
  return <>
    <main>
      ...
      <section className="engage section-pad"><div className="site-width"><div className="section-heading split"><div><span className="eyebrow">Choose the right starting point</span><h2>One problem. A clear first engagement.</h2></div><a className="text-link" href="/engagement-model">Compare engagement models <ArrowRight /></a></div><div className="engage-grid">{[['Automation Assessment','Map the process and leave with a prioritized implementation roadmap.','/process-automation'],['Defined Project','Design and deliver a specific product, workflow, or platform outcome.','/work'],['Startup Partnership','Explore qualified low-upfront product delivery with defined commercial rights.','/engagement-model']].map(([title,body,href],i)=><a href={href} key={title}><span>0{i+1}</span><h3>{title}</h3><p>{body}</p><b>Explore option <ArrowRight size={16}/></b></a>)}</div></div></section>
      ...
      <section className="final-cta"><div className="site-width"><span className="eyebrow eyebrow--light">Choose your next view</span><h2>Follow the problem before starting the conversation.</h2><p>Explore the operating workflow, the product experience, or the platform behind it—then contact us when the right starting point is clear.</p><div className="button-row"><a className="button button--white" href="/process-automation">Explore Process Automation <ArrowRight /></a><a className="button button--outline" href="/work">See Solution Patterns</a></div></div></section>
    </main>
  </>
}
...
function ServiceDepth({ service }: { service: string }) {
  ...
  if (service === 'Web & App Engineering') return <>
    ...
    <section className="audience-section section-pad" id="buyer-moments"><div className="site-width"><div className="section-heading split"><div><span className="eyebrow">When teams call us</span><h2>Four moments that need more than another developer.</h2></div><p>We enter where product ambition meets an operating constraint—then make the path to a dependable release visible.</p></div><div className="buyer-moments">{[
      ['01','Founders','The service works. The software does not exist yet.','Shape the first credible product.','/engagement-model','Explore the startup model'],
      ['02','Operations leaders','The team is moving faster than its tools.','Replace manual handoffs with one operating flow.','/process-automation','Explore process automation'],
      ['03','Product leaders','The roadmap is clear. Delivery capacity is not.','Add focused engineering without losing architecture.','/work','See delivery patterns'],
      ['04','Agencies & consultancies','The client need is real. The specialist bench is missing.','Extend delivery under clear ownership.','/about','Understand our approach'],
    ].map(([n,role,moment,outcome,href,cta])=><article className="buyer-moment" key={role}><div className="buyer-moment__top"><span>{n}</span><b>{role}</b></div><h3>{moment}</h3><p>{outcome}</p><a href={href}>{cta} <ArrowRight size={16}/></a></article>)}</div></div></section>
  </>
  return <>
...
function WorkPage() {
  ...
  return <main>
    ...
    <section className="work-showcases">{showcases.map((item,i)=><article className={`work-showcase ${i%2?'work-showcase--reverse':''}`} key={item.title}><div className="site-width work-showcase__grid"><div className="work-showcase__media"><img src={item.image} alt={item.alt} loading={i===0?'eager':'lazy'} /><span>Representative delivery environment</span></div><div className="work-showcase__copy"><div className="work-showcase__label"><span>{item.category}</span><b>{item.number}</b></div><h2>{item.title}</h2><p>{item.body}</p><div className="work-capabilities">{item.capabilities.map(capability=><span key={capability}><Check size={16}/>{capability}</span>)}</div><a className="text-link" href={item.href}>{item.cta} <ArrowRight /></a></div></div></article>)}</section>
...
function EngagementModelPage(){
  ...
  return <main>
    <section className="page-hero engagement-hero"><div className="site-width"><span className="eyebrow">How we engage</span><h1>Build now. Pay when it works.</h1><p>Selected startups get enterprise-grade product delivery without an upfront development fee. Ownership, success, and operating costs are agreed before we build.</p><div className="button-row"><a className="button button--primary" href="#commercial-model">Understand the partnership <ArrowRight /></a><a className="button button--secondary" href="/work">See delivery patterns</a></div><div className="engagement-summary"><span><b>£0 / $0</b>Upfront development*</span><span><b>Defined</b>Success milestone</span><span><b>Predictable</b>Operating cost</span><span><b>Licensed</b>Product access</span></div></div></section>
    ...
    <PageCta eyebrow="Product partnership" title="Have a credible problem and committed validation users?" copy="Share the product opportunity, founder commitment, and route to adoption so we can assess partnership fit." button="Apply for consideration" href="/contact?intent=product-partnership" />
  </main>
}
...
function PageCta({eyebrow='A practical next step',title='Bring us the workflow, not a preselected tool.',copy='We’ll help define what should be simplified, connected, automated, or built.',button='Discuss your project',href='/contact'}:{eyebrow?:string;title?:string;copy?:string;button?:string;href?:string}){return <section className="page-cta"><div className="site-width page-cta__panel"><div className="page-cta__copy"><span className="eyebrow eyebrow--light">{eyebrow}</span><h2>{title}</h2><p>{copy}</p></div><a className="button button--white" href={href}>{button} <ArrowRight/></a></div></section>}
